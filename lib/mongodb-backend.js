...
function _processKeysAndReturn(docs, cb) {
  var keyArrays = [];
  ...
  get : function(bucket, key, cb){
    ...
    this.db.collection(this.prefix + this.removeUnsupportedChar(collName),function(err,collection){
      ...
      collection.findOne(searchParams, {_bucketname: 0},function(err, doc){
        ...
        var keys = _.without(_.keys(doc),"key","_id","_bucketname");

        keys = async.map(keys, function(key, cb) {
          if (!/\*/.test(key)) return cb(null, key);

          var bucketName = new RegExp('^allows_' + key.replace(/\*/g, '.+') + '$');

          var searchParams = (that.useSingle? {_bucketname: bucketName}: {});
          var collName = (that.useSingle? aclCollectionName : bucketName);

          that.db.collection(that.prefix + collName,function(err,collection) {
            collection.find(searchParams, {_bucketname: 1, _id: 0}).toArray(cb);
          });
        }, function(err, docs) {
          if (err) return cb(err);
          var normalized = _(docs).
            flatten().
            map(function(d) {
              if (typeof d == 'string') return d;
              if (d._bucketname.slice(0, 7) == 'allows_') return d._bucketname.slice(7);
              return d._bucketname;
            }).
            uniq().
            filter(function(d) { return /^[^\*]+$/.test(d); }).value();
  ...
  union : function(bucket, keys, cb){
    ...
    this.db.collection(this.prefix + this.removeUnsupportedChar(collName),function(err,collection){
      ...
      collection.find(searchParams, {_bucketname: 0}).toArray(function(err,docs){
        ...
        if (docs.length || bucket == 'parents')
          return _processKeysAndReturn(docs, cb);

        var defaultRoles = ['roles', 'resources', 'parents'];

        collection.find({ key: {$in: keys}, _bucketname: {$nin: defaultRoles}}).toArray(function(err, docs) {
          var matches = docs.filter(function(doc) {
            var pattern1 = '^' + doc._bucketname.replace(/\*/g, '.+') + '$';
            var pattern2 = '^' + bucket.replace(/\*/g, '.+') + '$';
            return new RegExp(pattern1).test(bucket) || new RegExp(pattern2).test(doc._bucketname);
          });
          if (matches.length) _processKeysAndReturn(matches, cb);
          else cb(undefined, []);
        });
      });
    });
  ...
  add : function(transaction, bucket, key, values){
    ...
    transaction.push(function(cb){
      ...
      self.db.collection(self.prefix + self.removeUnsupportedChar(collName), function(err,collection){
        ...
        // update document
        collection.update(updateParams,{$set:doc, $setOnInsert: {createdAt: now}},{safe:true,upsert:true},function(err){
          if(err instanceof Error) return cb(err);
          cb(undefined);
        });
      });
  ...
  remove : function(transaction, bucket, key, values){
    ...
    transaction.push(function(cb){
      self.db.collection(self.prefix + self.removeUnsupportedChar(collName),function(err,collection){
        ...
        // update document
        collection.update(updateParams,{$unset:doc, $set: {updatedAt: new Date}},{safe:true,upsert:true},function(err){
          if(err instanceof Error) return cb(err);
          cb(undefined);
        });
      });
